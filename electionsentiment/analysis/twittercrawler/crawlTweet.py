...
search_congress = ['congress', 'gandhi', 'rahul']
search_bjp= ['modi', 'bjp', 'narendramodi']
search_both= ['congress', 'bjp']
search_anti =  ['rahulvsmodi', 'congressvsbjp']
search_countries = ['india','maldives', 'japan', 'africa', 'usa', 'canada', 'australia', 'vancouver', 'nigeria', 'philipines', 'finland', 'côte_d’ivoire']
...
searchQuery = ['#VotingRound3','#MyVoteForCongress',
'#CongAdmitsJhoot','#RahulApologizes','#BharatBoleNaMoNaMo', '#ChowkidarChorHai'
    ...
    for i in range(0, len(searchQuery)):
        ...
        while tweetCount < maxTweets:
            try:
                if (max_id <= 0):
                    if (not sinceId):
                        new_tweets = api.search(q=searchQuery[i],language = 'en', count=tweetsPerQry,tweet_mode='extended')
                    else:
                        new_tweets = api.search(q=searchQuery[i], language = 'en', count=tweetsPerQry,tweet_mode='extended',
                                                 since='2018-02-08', until='2019-04-22')
                else:
                    if (not sinceId):
                        new_tweets = api.search(q=searchQuery[i],language = 'en',  count=tweetsPerQry,tweet_mode='extended',
                                                max_id=str(max_id - 1))
                ...
                for tweet in new_tweets:
                    ...
                    clean_text = p.clean((tweet.full_text).translate(non_bmp_map))
                    #print(x.translate(non_bmp_map))
                    print(tweet.full_text.translate(non_bmp_map))



                    print("Clean Full text " + clean_text + "           " + tweet.full_text.translate(non_bmp_map) + str(x))
